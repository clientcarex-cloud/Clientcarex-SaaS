...
class Wikibooks_model extends App_Model
{
    ...
    public function get_all_books($query = ""){
        ...
        $sql = "
            SELECT 
                ...
                IFNULL(TWKCounters.total_views, 0) AS total_views,
                TWKCounters.latest_article_date,
                CONCAT(IFNULL(TBLAuthors.firstname, ''), ' ', IFNULL(TBLAuthors.lastname, '')) AS author_fullname
            FROM " . $tblBooks . " TBLBooks
                LEFT JOIN (
                    SELECT 
                        TBLArticles.book_id AS book_id, 
                        COUNT(*) as total,
                        IFNULL(SUM(TBLArticles.view_counter), 0) as total_views,
                        MAX(TBLArticles.updated_at) as latest_article_date
                    FROM " . $tblArticles . " TBLArticles
                    GROUP BY TBLArticles.book_id
                ) TWKCounters ON TBLBooks.id = TWKCounters.book_id
                LEFT JOIN " . $tblAuthors . " TBLAuthors ON TBLAuthors.staffid = TBLBooks.author_id
    ...
    /**
     * Get recent articles for a book (used in preview panel)
     */
    public function get_book_articles($book_id, $limit = 5){
        $tblArticles = db_prefix() . 'wiki_articles';
        $tblAuthors = db_prefix() . 'staff';
        ...
        $sql = "
            SELECT 
                TBLArticles.id,
                TBLArticles.title,
                TBLArticles.description,
                TBLArticles.type,
                TBLArticles.view_counter,
                TBLArticles.created_at,
                TBLArticles.updated_at,
                CONCAT(IFNULL(TBLAuthors.firstname, ''), ' ', IFNULL(TBLAuthors.lastname, '')) AS author_fullname
            FROM " . $tblArticles . " TBLArticles
                LEFT JOIN " . $tblAuthors . " TBLAuthors ON TBLAuthors.staffid = TBLArticles.author_id
            WHERE TBLArticles.book_id = " . (int)$book_id . "
            ORDER BY TBLArticles.updated_at DESC
            LIMIT " . (int)$limit . "
        ";

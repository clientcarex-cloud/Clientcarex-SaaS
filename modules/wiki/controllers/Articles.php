...
class Articles extends AdminController
{
    ...
    {
        if (!has_permission('wiki_articles', '', 'view') && !has_permission('wiki_books', '', 'view') && !is_admin()) {
    ...
    public function article($id = '')
    {
        ...
        if ($this->input->post()) {
            if ($id == '') {
                if (!has_permission('wiki_articles', '', 'create') && !has_permission('wiki_books', '', 'create') && !is_admin()) {
                    access_denied('wiki_articles');
                }
                ...
                if (!has_permission('wiki_articles', '', 'edit') && !has_permission('wiki_books', '', 'edit') && !is_admin()) {
                    access_denied('wiki_articles');
                }
                $data = $this->input->post();
                $data['content'] = $this->input->post('content', false);
        ...
        if ($id == '') {
            ...
            // Pre-select book from query param
            $preset_book_id = $this->input->get('book_id');
            if($preset_book_id){
                $data['preset_book_id'] = $preset_book_id;
            }
        } else {
    ...
    {
        if (!has_permission('wiki_articles', '', 'delete') && !has_permission('wiki_books', '', 'delete') && !is_admin()) {
            access_denied('wiki_articles');
        }
        if (!$id) {

...
{
    public function get_success_cancel_callback_url($subscription, $with_session = true)
    {
        $cancel_url = site_url('subscription/' . $subscription->hash);
        $success_url = current_url() . '?session_sub_hash=' . $subscription->hash . ($with_session ? "&session_id={CHECKOUT_SESSION_ID}" : '');
        return [$success_url, $cancel_url];
    }

    /**
     * Determine which checkout flow to use: setup mode or subscription mode
     * Uses only the global setting for simplicity
     *
     * @return string 'setup' or 'subscription'
     */
    protected function determine_checkout_flow()
    {
        // Check global setting
        $global_setting = get_option('perfex_saas_stripe_checkout_flow');

        // Return subscription mode if explicitly set, otherwise default to setup mode
        return $global_setting === 'subscription' ? 'subscription' : 'setup';
    }

    /**
     * Generate a client invoice.
     * Unified method that handles both setup and subscription checkout flows.
     *
    ...
    public function generate_company_invoice($clientid, $packageid, $custom_limitations)
    {
        ...
        $checkout_flow = $this->determine_checkout_flow();
        ...
        // Common subscription setup logic
        $subscription = $this->prepare_subscription_for_invoice($clientid, $packageid, $package);
        if (!$subscription) {
        ...
        try {
            ...
            // Prepare items
            $items = $this->prepare_subscription_items($package, $custom_limitations, $package_stripe_settings, $checkout_flow);

            $total = $package->price + $custom_limitations->total;
            if ($total > 0) {
                // Handle checkout session creation based on flow
                return $this->handle_checkout_flow($checkout_flow, $stripe_customer, $subscription, $package, $items, $package_stripe_settings, $custom_limitations);
            }
        } catch (\Throwable $th) {

            $stripe_session_id = $this->input->get('session_id');
    ...
    /**
     * Prepare subscription for invoice generation (common logic)
     *
     * @param int $clientid
     * @param int $packageid
     ...
     * @return object|null
     */
    protected function prepare_subscription_for_invoice($clientid, $packageid, $package)
    {
        ...
        hooks()->do_action('perfex_saas_company_subscription_generated', [
    ...
    /**
     * Prepare subscription items based on checkout flow
     *
     * @param object $package
     * @param object $custom_limitations
     * @param object $package_stripe_settings
     * @param string $checkout_flow
     * @return array
     */
    protected function prepare_subscription_items($package, $custom_limitations, $package_stripe_settings, $checkout_flow)
    {
        $default_stripe_price_id = $this->get_resources_price_id_from_settings($package_stripe_settings, 'default');

        if ($checkout_flow === 'subscription') {
            // For subscription mode, prepare line items for checkout session
            $items = [
                [
            ...
            // For setup mode, prepare items for subscription creation
            $items = [
                [
                    'price' => $default_stripe_price_id,
                    'quantity' => 1,
    ...
    /**
     * Handle checkout flow logic based on flow type
     *
     * @param string $checkout_flow
     ...
     * @return mixed
     */
    protected function handle_checkout_flow($checkout_flow, $stripe_customer, $subscription, $package, $items, $package_stripe_settings, $custom_limitations)
    {
        // Allow for custom redirection
        if (empty($this->input->get('session_sub_hash'))) {
            list($success_url, $cancel_url) = $this->get_success_cancel_callback_url($subscription, false);
            $filter = hooks()->apply_filters(
                'perfex_saas_before_stripe_setup_session',
                ['subscription' => $subscription, 'success_url' => $success_url, 'cancel_url' => $cancel_url]
            );
            if (!empty($filter['action_url'] ?? '')) {
        ...
        $stripe_session_id = $this->input->get('session_id');
        if (!empty($stripe_session_id)) {
            $session = $this->finalize_stripe_checkout_session($stripe_customer, $stripe_session_id);
        }

        // Force setup mode is user already have a default payment method attach and not finalizing a session
        if (empty($stripe_session_id) && $this->stripe_customer_has_default_payment_method($stripe_customer)) {
            $checkout_flow = 'setup';
        }

        // Handle subscriptin checkout mode
        if ($checkout_flow === 'subscription') {

            if (
                empty($stripe_session_id)
            ) {
                // Subscription mode checkout session
                $session = $this->create_subscription_checkout_session(
                    $stripe_customer,
                    $subscription,
                    $package,
                    $items,
                    $package_stripe_settings
                );
            ...
            // An active session , get the stripe subscription from it
            if (!empty($stripe_session_id)) {

                $stripeSubscription = isset($session->subscription) && is_object($session->subscription) ?
                    $session->subscription :
                    $this->stripe_subscriptions->get_subscription($session->subscription);
            }
        } else { // Setup mode flow (original logic)

            $stripeSubscription = $this->handle_setup_mode_flow($stripe_customer, $subscription, $package, $items, $package_stripe_settings, $custom_limitations);
            if (isset($stripeSubscription->action_url)) {
                // return if action needed
                return $stripeSubscription;
            }
        }

        // We must now have a stripe subscription
        if (!isset($stripeSubscription)) throw new Exception("Error finding stripe subscription", 1);

        // Link to the local subscription
        $this->subscriptions_model->update($subscription->id, [
            'status' => $stripeSubscription->status,
            'stripe_subscription_id' => $stripeSubscription->id,
        ]);

        // Link to the client metadata saas
        perfex_saas_get_or_save_client_metadata($subscription->clientid, [
            'subscription_id' => $subscription->id,
            'subscription_package_id' => $package->id,
        ]);

        // If invoice require further action
     ...
     * @return mixed
     */
    protected function handle_setup_mode_flow($stripe_customer, $subscription, $package, $items, $package_stripe_settings, $custom_limitations)
    {
        $stripe_session_id = $this->input->get('session_id');
        if (!$this->stripe_customer_has_default_payment_method($stripe_customer) && empty($stripe_session_id)) {
    ...
    public function update_company_invoice($invoice, $clientid, $custom_limitations)
    {
        ...
        if ($total > 0) {
            ...
            // Get the customer payment method if not yet provided through stripe session
            if (!$this->stripe_customer_has_default_payment_method($stripe_customer) && empty($session_sub_hash)) {
                list($success_url, $cancel_url) = $this->get_success_cancel_callback_url($subscription);
                $session = $this->get_stripe_checkout_session(
                    $stripe_customer,
                    $subscription->name,
    ...
    protected function get_stripe_customer($clientid)
    {
        ...
        if (!$customer && !empty($customerPayload['email'])) {
    ...
    /**
     * Check if the customer has payment on file i.e has attached payment method.
     *
     * @param \Stripe\Customer $stripe_customer
     * @return bool
     */
    public function stripe_customer_has_default_payment_method($stripe_customer)
    {
     ...
     * @return \Stripe\Checkout\Session
     */
    public function create_subscription_checkout_session($stripe_customer, $subscription, $package, $line_items, $package_stripe_settings)
    {
        list($success_url, $cancel_url) = $this->get_success_cancel_callback_url($subscription, true);
        ...
        $session_data = [
            ...
            'subscription_data' => $this->create_subscription_data($subscription, $package_stripe_settings),
        ];

        // Add customer information
        if ($stripe_customer->id) {
            $session_data['customer'] = $stripe_customer->id;
        } else {
            $session_data['customer_email'] = $stripe_customer->email;
        }
        ...
        if (!empty($subscription->terms)) {
            $session_data['consent_collection']['terms_of_service'] = 'required';
            $session_data['custom_text']['terms_of_service_acceptance']['message'] = clear_textarea_breaks($subscription->terms);
        }

        // Add custom submit message
        if (!empty($subscription->description)) {
            $session_data['custom_text']['submit']['message'] = clear_textarea_breaks($subscription->description);
        } else {
            $session_data['custom_text']['submit']['message'] = 'Subscribe to ' . $package->name;
        }

        $session_data = hooks()->apply_filters('perfex_saas_stripe_subscription_data', $session_data);
    ...
    /**
     * Create subscription data for Stripe subscription
     *
     * @param object $subscription
     * @param object $package_stripe_settings
     * @return array
     */
    protected function create_subscription_data($subscription, $package_stripe_settings)
    {
        $params = [];
        ...
        // Handle future billing cycles and proration
        $proration_behavior = $package_stripe_settings->proration_behavior ?? 'none';

        // Invalid subscription_data[proration_behavior]: must be one of create_prorations or none
        $proration_behavior = $proration_behavior == 'create_prorations' ? $proration_behavior : 'none';
    ...
    public function finalize_stripe_checkout_session($stripe_customer, $stripe_session_id)
    {
        ...
        if (isset($session->setup_intent->payment_method)) {
            $payment_method = $session->setup_intent->payment_method;
        } else if (isset($session->subscription->default_payment_method)) {
            $payment_method = \Stripe\PaymentMethod::retrieve($session->subscription->default_payment_method);
        } else {
            return $session;
        }
        ...
        if ($payment_method) {
            ...
            $this->stripe_subscriptions->update_customer($stripe_customer->id, [
                'invoice_settings' => [
    ...
    protected function create_stripe_subscription($subscription, $items, $stripe_customer, $proration_behavior, $taxes = [])
    {
        ...
        $params = hooks()->apply_filters('perfex_saas_stripe_subscription_data', $params);

        $stripeSubscription = $this->stripe_subscriptions->subscribe($stripe_customer->id, $params);

        hooks()->do_action('perfex_saas_stripe_subscription_updated', [
            'params' => $params,
    ...
    protected function update_stripe_subscription($subscription_id, $update_values, $proration_behavior, $taxes = [])
    {
        ...
        $update_data = hooks()->apply_filters('perfex_saas_stripe_subscription_data', $update_data);

        $stripeSubscription = \Stripe\Subscription::update($stripeSubscription->id, $update_data);

...
trait Perfex_saas_subscription_trait
{
    ...
    public function generate_company_invoice($clientid, $packageid, $extra_custom_data = [])
    {
        ...
        hooks()->do_action('perfex_saas_company_invoice_generated', [
            'invoice' => $invoice,
            'old_invoice' => $old_invoice,
            'old_customization' => $old_customization,
            'on_trial' => $on_trial,
            'package' => $package
        ]);
    ...
    public function cancel_company_invoice(int $clientid, $type = 'immediately', $invoice = null)
    {
        ...
        if (!empty($invoice->subscription_id)) {
            ...
            if ($cancelled_invoice && $type == 'at_period_end') {
                perfex_saas_get_or_save_client_metadata(
                    $clientid,
                    ['last_cancelled_invoice' => $cancelled_invoice->subscription_id]
                );
            }
        ...
        if ($type == 'at_period_end') {
            // Normal recurring invoice. only save metadata
            perfex_saas_get_or_save_client_metadata(
                $clientid,
                ['will_cancel_invoice_at_period_end' => $invoice->id]
            );
            return $invoice;
        }
        ...
        perfex_saas_get_or_save_client_metadata(
            $clientid,
            [
                'last_cancelled_invoice' => $invoice->id,
                'last_cancelled_invoice_recurring' => $invoice->recurring,
                'subscription_id' => '',
                'will_cancel_invoice_at_period_end' => ''
            ]
        );
    ...
    public function resume_company_invoice(int $clientid)
    {
        ...
        // Resume promised cancel invoice for standard recurring invoice
        if (!empty($client_metadata->will_cancel_invoice_at_period_end ?? '') && !$invoice) {
            $invoice = $this->get_company_invoice($clientid);
            if (empty($invoice->subscription_id)) {
                perfex_saas_get_or_save_client_metadata(
                    $clientid,

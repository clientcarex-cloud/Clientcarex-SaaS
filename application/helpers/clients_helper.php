...
function load_client_language($customer_id = '')
{
    ...
    if (defined('CLIENTS_AREA') && get_contact_language() != '' && !is_language_disabled()) {
        ...
        if (empty($language) || !file_exists(APPPATH . 'language/' . $language)) {
            $language = 'english';
        }
...
function send_customer_registered_email_to_administrators($client_id)
{
    ...
    if (!$smtp_configured) {
        @file_put_contents($debugLogFile, '[' . date('Y-m-d H:i:s') . '] HELPER: SMTP not configured, skipping admin notification emails for client_id=' . $client_id . PHP_EOL, FILE_APPEND | LOCK_EX);
        return;
    }
    ...
    foreach ($admins as $admin) {
        try {
            @file_put_contents($debugLogFile, '[' . date('Y-m-d H:i:s') . '] HELPER: Sending admin notification to ' . $admin['email'] . PHP_EOL, FILE_APPEND | LOCK_EX);
            send_mail_template('customer_new_registration_to_admins', $admin['email'], $client_id, $admin['staffid']);
            @file_put_contents($debugLogFile, '[' . date('Y-m-d H:i:s') . '] HELPER: Admin notification sent OK to ' . $admin['email'] . PHP_EOL, FILE_APPEND | LOCK_EX);
        } catch (\Throwable $e) {
            @file_put_contents($debugLogFile, '[' . date('Y-m-d H:i:s') . '] HELPER: Admin notification FAILED to ' . $admin['email'] . ' - ' . get_class($e) . ': ' . $e->getMessage() . PHP_EOL, FILE_APPEND | LOCK_EX);
            log_activity('Failed to send registration notification to admin ' . $admin['staffid'] . ': ' . $e->getMessage());
        }
    }
}
